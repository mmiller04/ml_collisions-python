...
# !gcloud config set project {project_id}
# !gsutil cp -r gs://ml-collisions-data1/hdf5_data/hdf_cons_fullvol.h5 /content/hdf5_data
...
# plt.plot(c[2])
# plt.ylim([1e-3,1e0])
# plt.legend(['l2_loss','den_loss','mom_loss','en_loss'])
# plt.yscale('log')
...
fid = open('/content/lr_new_tune.txt','w')
fid.write('lr: '+str(lr)+'\n')
...
plt.xlabel('iterations')
plt.title('ReSeg training, {} epochs, iphi=0'.format(num_epochs))
plt.legend(['l2','conservation'])
plt.savefig('l2_cons.png')
files.download('l2_cons.png')

for epoch in range(num_epochs):
  files.download('figs/dfs_{}.png'.format(epoch+1))

#len(loss_vector)
#iterations1 = np.linspace(1,4,4)
cons_array_i = cons_array

cons_array.shape

cons_array_np = np.zeros([41670,6])

for i in range(10):
  cons_array_np[4167*i:4167*(i+1)] = cons_array[:,i*6:(i+1)*6]
  
cons_array_np=cons_array_np.transpose()
...
for lim in range(num_epochs):
  plt.plot(cons_array_np[0,lim*num_each_epoch:(lim+1)*num_each_epoch],\
           cons_array_np[1,lim*num_each_epoch:(lim+1)*num_each_epoch],'o')

min_dens = cons_array_np[:2].min()
max_dens = cons_array_np[:2].max()

plt.plot(np.linspace(min_dens,max_dens,100),np.linspace(min_dens,max_dens,100))
plt.ylabel('mass before')
plt.xlabel('mass after')
plt.title('Mass conservation, {} epochs, iphi=0'.format(num_epochs))
plt.legend(['epoch{}'.format(epoch) for epoch in range(num_epochs)])
plt.savefig('mass.png')
#files.download('mass.png')
...
plt.plot(np.linspace(-1.5e19,-1e18,100),np.linspace(-1.5e19,-1e18,100))
plt.ylabel('momentum before')
plt.xlabel('momentum after')
...
for lim in range(num_epochs):
  plt.plot(cons_array_i[4,lim*num_each_epoch:(lim+1)*num_each_epoch],\
           cons_array_i[5,lim*num_each_epoch:(lim+1)*num_each_epoch],'o')
plt.plot(np.linspace(-7e19,-1e19,100),np.linspace(-7e19,-1e19,100))
plt.ylabel('energy before')
plt.xlabel('energy after')
plt.title('Energy conservation, {} epochs, iphi=0'.format(num_epochs))
plt.legend(['epoch{}'.format(epoch) for epoch in range(num_epochs)])
plt.savefig('energy.png')
# files.download('energy.png')

plt.contourf(df[20000,0,:,:])
plt.colorbar()
